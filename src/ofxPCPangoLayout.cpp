...
ofxPCPangoLayout::ofxPCPangoLayout(ofxPCContext *pContext) : context(pContext) {
  ...
  cout << "ctor pa_layout" << pa_layout << endl;
  cout << "ctor pContext->cr_context" << pContext->cr_context << endl;
}
...
void ofxPCPangoLayout::show() {
  // do_move_to_things(0, 0);

  //  cairo_move_to(context->cr_context, 0, 0);

  //  cout << context->cr_context << endl;

  //  pango_cairo_show_layout(context->cr_context, pa_layout);

  //  cairo_save(context->cr_context);
  //  cairo_move_to(context->cr_context, 0, 0);

  cout << "show pa_layout" << pa_layout << endl;
  cout << "show pContext->cr_context" << context->cr_context << endl;
  pango_cairo_update_layout(context->cr_context, pa_layout);
  pango_cairo_show_layout(context->cr_context, pa_layout);

  cairo_surface_write_to_png(context->getSurface()->cr_surface, "~/Desktop/sadfasdfasdf.png");

  //  pango_cairo_show_layout(context->cr_context, pa_layout);
  //  cairo_restore(context->cr_context);
  //  cairo_surface_flush(cairo_get_target(context->cr_context));
}
